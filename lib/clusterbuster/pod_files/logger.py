...
class logger_client(clusterbuster_pod_client):
    ...
    def __init__(self):
        try:
            ...
            self.__bytes_per_line = clusterbuster_pod_client.toSize(self._args[2])
            self.__lines_per_io = clusterbuster_pod_client.toSize(self._args[3])
            self.__xfer_count = clusterbuster_pod_client.toSize(self._args[4])
    ...
    def runit(self, process: int):
        ...
        while ((self.__xfer_time == 0 and self.__xfer_count == 0) or
               (self.__xfer_time > 0 and self.adjusted_time(data_start_time) < self.__xfer_time) or
        ...
        self.report_results(data_start_time, data_end_time, data_end_time - data_start_time, user, system, extras)
