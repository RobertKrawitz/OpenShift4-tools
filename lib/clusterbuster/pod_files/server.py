        ...
        while True:
            while ntotal < self.msg_size:
                try:
                    ...
                    self.timestamp(f"Recv failed: {err}")
                    os._exit(1)
                if len(answer) == 0:
                    if consec_empty > 1:
                        self.timestamp(f"Exiting {conn}")
                    os._exit(0)
            ...
            while ntotal > 0:
                try:
                    ...
                    self.timestamp(f"Send failed: {err}")
                    os._exit(1)
                ntotal -= answer

    def runit(self, process: int):
        expected_clients = self.expected_clients
        sock = self.listen(None, port=self.listen_port, backlog=self.expected_clients)
        ...
        while expected_clients > 0:
            try:
                ...
                self.timestamp(f"Accept failed: {err}")
                return 1
            try:
                child = os.fork()
                if child == 0:
                    sock.close()
                    self.timestamp(f"Accepted connection from {address}")
        ...
        self.timestamp("Waiting for all clients to exit:")
        while pid_count > 0:
            try:
                pid, status = os.wait()
